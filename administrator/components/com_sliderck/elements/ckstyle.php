...
JText::script('SLIDERCK_SAVE_CLOSE');
require_once JPATH_ROOT . '/administrator/components/com_sliderck/helpers/helper.php';
...
class JFormFieldCkstyle extends JFormField {
	...
	protected function getInput() {
		...
		$html .= '<div class="ckbutton" onclick="CKBox.open({url: \'index.php?option=com_sliderck&view=styles&tmpl=component&layout=modal\', style: {padding: \'0px\'}})"><i class="fas fa-mouse-pointer "></i> ' . JText::_('SLIDERCK_SELECT') . '</div>';
		$html .= '<div class="ckbutton" onclick="CKBox.open({url: \'index.php?option=com_sliderck&view=style&tmpl=component&layout=modal&id=\'+jQuery(\'#' . $this->id . '\').val()+\'\'})"><i class="fas fa-edit"></i> ' . JText::_('SLIDERCK_EDIT') . '</div>';
		$html .= '<div class="ckbutton cktip" onclick="jQuery(\'#' . $this->id . '\').val(\'\');jQuery(\'#' . $this->id . 'name\').val(\'\');" title="' . JText::_('SLIDERCK_REMOVE') . '"><i class="fas fa-times"></i></div>';
		$html .= '</div>';
