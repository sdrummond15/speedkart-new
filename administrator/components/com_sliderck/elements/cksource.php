...
class JFormFieldCksource extends JFormFieldList
{
	...
	protected function getOptions() {
		...
		if (JPluginHelper::isEnabled('system', 'sliderck')) {
			// load the custom plugins
			...
			if (count($sources)) {
				foreach ($sources as $source) {
					...
					if (! $this->findOption($source)) {
						$tmp = JHtml::_(
							'select.option', (string) $source, JText::alt(trim((string) 'SLIDERCK_SOURCE_' . strtoupper($source)), preg_replace('/[^a-zA-Z0-9_\-]/', '_', $this->fieldname)), 'value', 'text', '0'
						);
						// Add the option object to the result set.
						$this->options[] = $tmp;
					}
