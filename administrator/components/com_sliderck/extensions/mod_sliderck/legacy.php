...
// $captionheight = modSliderckHelper::testUnit($params->get('captionheight', '100'));
...
if ($source != 'folder') {
	...
	include_once JPATH_ROOT . '/administrator/components/com_sliderck/helpers/source/' . $source . '.php';
}
...
if (file_exists(JPATH_SITE . '/plugins/system/mediabox_ck/mediabox_ck.php') && $params->get('lightbox', 'mediabox') == 'mediabox') {
	...
	if (! empty($plugin)) {
		...
		if ($lightboxParams->get('attribtype') == 'className') {
			...
			$lightboxAttribs = ' rel="' . $lightboxParams->get('attribname')  .'[sliderck' . $module->id . ']"';
		}
	}
}
...
// load custom styles
$css = '';
$css .= $params->get('height', 'fixed') == 'fixed' ? '#sliderck' . $module->id . ' { height: ' . modSliderckHelper::testUnit($params->get('heightvalue', '250')) . '; }' : '';
if ( $params->get('forceimageheight', '1') == '1') {
	$css .= $params->get('imageheight', '') ? '#sliderck' . $module->id . ' .swiper-slide .swiper-img { height: ' . modSliderckHelper::testUnit($params->get('imageheight', '')) . ' }' : '';
} 
...
if ($params->get('pagination', 'bullets') == 'thumbs') {
	$css .= '#sliderck' . $module->id . 'thumbs { height: ' . modSliderckHelper::testUnit($params->get('thumbsheight', '100')) . '; }
#sliderck' . $module->id . 'thumbs .swiper-slide { width: ' . (100 / (int) $params->get('thumbsnumber', '5')) . '%; }
';
}

$layoutcss = str_replace('|ID|', '#sliderck' . $module->id, modSliderckHelper::getStyleLayoutcss($params->get('styles', '')) );
$css .= $layoutcss;
