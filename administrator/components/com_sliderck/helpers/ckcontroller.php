...
class CKController {
	...
	// public function getModel($base = '\Sliderck\CKModel') {
		// if (empty($this->model)) {
			// $name = $this->getName();
			// require_once(SLIDERCK_PATH . '/helpers/ckmodel.php');
			// require_once(SLIDERCK_PATH . '/models/' . strtolower($name) . '.php');
	...
	protected function createView($name, $prefix = '')
	{
		...
		{
			$path = SLIDERCK_PATH . '/views/' . $this->createFileName('view', array('name' => $viewName));
	...
	public function edit($id = null, $appendUrl = '') {
		...
		CKFof::redirect(SLIDERCK_ADMIN_URL . '&view=' . $this->getName() . '&layout=edit&id=' . $editId . $appendUrl);
	}
	...
	public function copy() {
		...
		// Redirect to the edit screen.
		CKFof::redirect(SLIDERCK_ADMIN_URL);
	}
